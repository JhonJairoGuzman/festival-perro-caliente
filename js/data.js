...
const festivalData = {
    ...
    perros: [
        { 
            ...
            ingredientes: ["Pan Brioche 25cm", "Salsa de la casa", "Papa ripio", "Chorizo de ternera", "Salchicha ranchera", "Pollo", "Cerdo", "Cebolla caramelizada en salsa BBQ", "Triple reacción de queso mozzarella", "Maíz tierno", "Jamón ahumado", "Tocineta", "Salsa especial ahumada"],
            negocio: "the renacer",
...
// 3. TERCERO: Funciones para interactuar con Supabase
...
async function addVote(hotdogId) {
    if (!supabase) {
        console.error('❌ Supabase no disponible, no se puede votar');
        return false;
    }
...
async function saveComment(name, comment) {
    if (!supabase) {
        console.error('❌ Supabase no disponible, no se puede guardar comentario');
        return false;
    }
...
async function getComments() {
    if (!supabase) {
        console.error('❌ Supabase no disponible, usando comentarios locales');
        return [];
    }
...
async function initializeDatabase() {
    if (!supabase) {
        console.error('❌ Supabase no disponible, no se puede inicializar BD');
        return;
    }
